...
#filter username for letters and numbers only (prevent sql injection)
...
if not data:
    if "y" in input('Account with username %s not found. Would you like to create a new account? [y/n]' %username).lower():
        ...
        #filter email for letters, numbers, and accepted characters only (prevent sql injection)
        c.execute("INSERT INTO accounts values (?,?,?,?,?)", [username, passwordHash, email, int(time.time()), int(time.time())]) #set date accessed and date created to current time
